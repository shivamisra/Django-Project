...
from .models import DailySub,Requisition,Offer , GroupMaster
...
class DailySubSerializer(serializers.ModelSerializer):
    ...
    class Meta:
            model = DailySub
            fields=['id','businessGroup','sourceDate','sourcer','recruiter','manager','rrID','jobLocation','practice','skill','source','source2','fullName','candidateName','emailID','contactNo','currentLocation','experienceIn','currentOrg','currentCTC','expectedCTC','noticePeriod','l1InterviewDate','l1Interviewer','l2InterviewDate','l2Interviewer','l3InterviewDate','l3Interviewer','status','feedback','dateOfOffer','dateOfJoining','declineReasons','dateOfDecline','week','recordingLinkL1','recordingLinkL2','remarks']
...
class OfferSerialize(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        instance.recruiter = validated_data.get('recruiter', instance.recruiter)
        instance.manager = validated_data.get('manager', instance.manager)
        instance.dateOfOffer = validated_data.get('dateOdOffer', instance.dateOfOffer)
        instance.dateOfJoining = validated_data.get('dateOfJoining', instance.dateOfJoining)
        instance.rrID = validated_data.get('rrID', instance.rrID)
...
class RequisitionSerializer(serializers.ModelSerializer):
    ...
    # tat=serializers.IntegerField()
    # openAsOnDate=serializers.IntegerField()

    class Meta:
        model=Requisition
        fields=['id','skill','superClass','jobFamily','yearOfExp','grade','status','currentWeekPriority','hiringManager',
        'hod','projectClient','rrID','practice','allocatedResource','PMOStatus','recruiter','permSubconContract',
        'requisitionRaiseDate','requirement','profilesShared','screenReject','shortlistedforInterviews','selected',
        'offersReleased','joined','yetToJoin','decline','tat','openAsOnDate', 'days']
   

    # def openAsOnDate(value):
    #     if value.requirement < value.joined:
    #         raise serializers.ValidationError("Date entered is incorrect")
    #     return value.requirement - value.joined


    # def tat(value):
    #     if value.requisitionRaiseDate > value.datetime.date.today():
    #         raise serializers.ValidationError("Date entered is incorrect")
    #     return value.datetime.date.today() - value.requisitionRaiseDate
    ...
    def update(self,instance, validated_data):
        ...
        instance.shortlistedforInterviews = validated_data.get('shortlistedforInterviews', instance.shortlistedforInterviews)
